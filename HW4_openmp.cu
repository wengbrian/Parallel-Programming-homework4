...
void block_FW(int B)
{
    ...
    #pragma omp parallel
    {
        ...
        cudaSetDevice(tid);
        // copy memory to host
        cudaMallocPitch(&(d_ptr[tid]), &(pitch[tid]), n * sizeof(int), n);
        cudaMemcpy2D(d_ptr[tid], pitch[tid], &Dist, V * sizeof(int), n * sizeof(int), n, cudaMemcpyHostToDevice);
        for (int r = 0; r < round; ++r) {
            if(tid == 0){
                ...
                if(r/2 > 0){
                    kernel(r, r, 0, 1, r/2, n, d_ptr[tid], pitch[tid]); // outer-up
                }
                if((round-r-1)/2 > 0){
                    kernel(r, r, r+1+(round-r-1)/2+(round-r-1)%2, 1, (round-r-1)/2, n, d_ptr[tid], pitch[tid]); // outer-down
                }
                if(r/2 > 0){
                    kernel(r, 0, r, r/2, 1, n, d_ptr[tid], pitch[tid]); // outer-left
                }
                if((round-r-1)/2 > 0){
                    kernel(r, r+1+(round-r-1)/2+(round-r-1)%2, r, (round-r-1)/2, 1, n, d_ptr[tid], pitch[tid]); // outer-right
                }
                // TODO
                // async memory copy
                ...
                if(r/2 > 0){
                    kernel(r, 0, 0, r/2, r/2, n, d_ptr[tid], pitch[tid]); // outer-left-up
                }
                if(((round-r-1)/2 > 0) && (r/2 > 0)){
                    kernel(r, 0, r+1+(round-r-1)/2+(round-r-1)%2, r/2, (round-r-1)/2, n, d_ptr[tid], pitch[tid]); // outer-left-down
                }
                if(((round-r-1)/2 > 0) && (r/2 > 0)){
                    kernel(r, r+1, 0, (round-r-1)/2, r/2, n, d_ptr[tid], pitch[tid]); // outer-right-up
                }
                if((round-r-1)/2 > 0){
                    kernel(r, r+1+(round-r-1)/2+(round-r-1)%2, r+1+(round-r-1)/2+(round-r-1)%2, (round-r-1)/2, (round-r-1)/2, n, d_ptr[tid], pitch[tid]); // right-down
                }
                // TODO
                ...
                //    printf("kernel launch failed with error \"%s\".\n",cudaGetErrorString(cudaerr));
                if((r/2 > 0) && (r/2+r%2 > 0)){
                    kernel(r, 0, r/2, r/2, r/2+r%2, n, d_ptr[tid], pitch[tid]); // outer-left-up-grid
                }
                if((r/2 > 0) && ((round-r-1)/2+(round-r-1)%2 > 0)){
                    kernel(r, 0, r+1, r/2, (round-r-1)/2+(round-r-1)%2, n, d_ptr[tid], pitch[tid]); // outer-left-down-grid
                }
                if(((round-r-1/2) > 0) && (r/2+r%2 > 0)){
                    kernel(r, r+1+(round-r-1)/2+(round-r-1)%2, r/2, (round-r-1)/2, r/2+r%2, n, d_ptr[tid], pitch[tid]); // outer-right-up-grid
                }
                if(((round-r-1/2) > 0) && ((round-r-1)/2+(round-r-1)%2 > 0)){
                    kernel(r, r+1+(round-r-1)/2+(round-r-1)%2, r+1, (round-r-1)/2, (round-r-1)/2+(round-r-1)%2, n, d_ptr[tid], pitch[tid]); // outer-right-down-grid
                }
            }else if(tid == 1){
                // inner 
            }
