...
void block_FW(int B)
{
	...
	for (int r = 0; r < round; ++r) {
        ...
        if((round-r-1 > 0) && (r > 0)){
            int num_block_width = ceil(B*r,32);
            int num_block_height = ceil(B*(round-r-1)*r,32);
            dim3 grid(num_block_width, num_block_height);
...
{
    int size = B <= 32 ? B : 32; // block width/height
    ...
    int idx = threadIdx.y * blockDim.y + threadIdx.x; // internal index, used by dist
    ...
    // copy current block
    if ((tidx < n) && (tidy < n) && (threadIdx.x < block_width*B) && (threadIdx.y < block_height*B)){
    ...
    for (int i = 0; i < num_block; i++){
        ...
        if((idx_x < (Round+1)*B) && (idx_x < n) && (threadIdx.x < block_width*B) && (threadIdx.y < block_height*B)){
            int *row = (int *)((char*)d_ptr + tidy * pitch);
        ...
        if((idx_y < (Round+1)*B) && (idx_y < n) && (threadIdx.x < block_width*B) && (threadIdx.y < block_height*B)){
        ...
        for(int k = 0; (k < size) && (Round*B+k < n); k++){
            if ((tidx < n) && (tidy < n) && (threadIdx.x < block_width*B) && (threadIdx.y < block_height*B)){
                ...
                if(debug)printf("[%d,%d][epoch=%d, k=%d, ex_tidx=%d, ex_tidy=%d, in_tidx=%d, in_tidy=%d, idx=%d] if %d + %d < %d\n", block_width*B, block_height*B, i, Round*B+i, tidx, tidy, threadIdx.x, threadIdx.y, idx, ik, kj, dist[idx]);
                if (ik + kj < dist[idx]) {
                    dist[idx] = ik + kj;
                }
            }
